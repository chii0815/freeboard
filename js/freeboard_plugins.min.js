	...
	this.setEditing = function (a, b) {
		if (d.allow_edit() || !a) {
			...
			a ? ($("#toggle-header-icon").addClass("icon-chevron-up").removeClass("icon-wrench"), $(".gridster .gs_w").css({
					...
					top: "20"
				}, e), $("#main-header").data().shown = !1, $(".sub-section").unbind(), c.disableGrid()),
...
function WidgetModel(a, b) {
	...
	this.sendValue = function(si,sv){
		if(si){
			var matches = si.match(/datasources\[["'](\w+)["']\]/);
			if(matches){
				_.each(a.datasources(), function(d){
					if(d.name() == matches[1]){
						if(d.datasourceInstance.sendvalues && _.isFunction(d.datasourceInstance.sendvalues)){
							d.datasourceInstance.sendvalues(sv);
						}
					}
				})	
			}
				...
				if ("calculated" == a.type) {
					...
					if (!_.isUndefined(b)) {
						...
						for (var i; i = c.exec(b); ) {
							...
							_.isUndefined(k) && (k = [], d.datasourceRefreshNotifications[j] = k),
	...
	this.render = function (a) {
		d.shouldRender(!1),
		!_.isUndefined(d.widgetInstance) && _.isFunction(d.widgetInstance.render) && (d.widgetInstance.render(a), d.updateCalculatedSettings())
	},
	this.dispose = function () {},
	this.serialize = function () {
		return {
	...
	function g(a, b, g, h, i) {
			...
			_.each(a, function (a) {
				...
				function e(b) {
					...
					c.append($('<td class="table-row-operation"></td>').append($('<ul class="board-toolbar"></ul>').append($("<li></li>").append($('<i class="icon-trash icon-white"></i>').click(function () {
										...
										-1 != b && (m.settings[a.name].splice(b, 1), c.remove(), d())
									}))))),
				...
				case "option":
					...
					_.each(a.options, function (a) {
						var b,
						c;
						_.isObject(a) ? (b = a.name, c = a.value) : b = a,
						_.isUndefined(c) && (c = b),
						_.isUndefined(t) && (t = c),
		...
		if (q.length > 1) {
			...
			p.change(function () {
				...
				d(),
				l = b[p.val()],
				_.isUndefined(l) ? ($("#setting-row-instance-name").hide(), $("#dialog-ok").hide()) : ($("#setting-row-instance-name").show(), l.description && l.description.length > 0 ? o.html(l.description).show() : o.hide(), $("#dialog-ok").show(), k(l.settings, l.typeahead_source, l.typeahead_data_segment))
			})
		} else
...
var freeboard = function () {
		...
		init: function (a, f, g, i, j) {
			...
			$(a).click(function (f) {
				if ("delete" == k.operation) {
					...
					var j = void 0;
					"datasource" == k.type ? "add" == k.operation ? m = {}
					 : (j = i.type(), m = i.settings(), m.name = i.name()) : "widget" == k.type ? "add" == k.operation ? m = {}
					 : (j = i.type(), m = i.settings()) : "pane" == k.type && (m = {}, "edit" == k.operation && (m.title = i.title(),m.icon=i.icon(), m.col_width = i.col_width()), l = {
							settings: {
								settings: [{
										name: "title",
										display_name: "Title",
					...
					h.createPluginEditor(n, l, j, m, function (f) {
						if ("add" == k.operation) {
							...
							"edit" == k.operation && ("pane" == k.type ? (i.title(f.settings.title), i.icon(f.settings.icon), i.col_width(f.settings.col_width), d.processResize(!1)) : ("datasource" == k.type && (i.name(f.settings.name), delete f.settings.name), i.type(f.type), i.settings(f.settings)))
					})
				}
			})
		}
	...
	$(function () {
		...
		addStyle: function (a, b) {
			var c = a + "{" + b + "}",
			d = $("style#fb-styles");
			0 == d.length && (d = $('<style id="fb-styles" type="text/css"></style>'), $("head").append(d)),
...
();
$.extend(freeboard, jQuery.eventEmitter), function () {
	var a = function (a, b) {
		function c(a) {
			e && clearInterval(e),
...
(), function () {
	...
	var g = function (b) {
		...
		this.onSettingsChanged = function (a) {
			...
			b ? (g.html(_.isUndefined(a.title) ? "" : a.title), g.attr("style", null)) : (g.empty(), g.hide()),
			c ? (i.html(_.isUndefined(a.units) ? "" : a.units), i.attr("style", null)) : (i.empty(), i.hide());
			var f = 30;
	...
	var i = function (a) {
		...
		this.onSettingsChanged = function (a) {
			a.min_value != i.min_value || a.max_value != i.max_value || a.units != i.units ? (i = a, b()) : i = a,
			e.html(a.title)
		},
		...
		this.render = function (a) {
			function e() {
				...
				google.maps.event.addDomListener(a, "mouseenter", function (a) {
					a.cancelBubble = !0,
					c.hover || (c.hover = !0, c.setOptions({
							zoomControl: !0
						}))
				}),
